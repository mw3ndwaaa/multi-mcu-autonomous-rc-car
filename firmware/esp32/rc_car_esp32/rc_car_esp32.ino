/*
  Multi-MCU Autonomous RC Car - ESP32 NodeMCU / ESP32-WROOM-32 Version
  --------------------------------------------------------------------
  Functional port of the Arduino Mega baseline firmware.
  ...
  Features:
    ...
    - Hardware UART2 reserved/enabled for a future external Bluetooth module

  IMPORTANT 3.3 V LOGIC NOTE
  --------------------------
  ESP32 GPIO is 3.3 V only. Do not feed 5 V into ESP32 GPIO.
  ...
  - If IR modules are powered from 5 V, level-shift/divide their OUT signals,
    or power compatible IR modules from 3.3 V so their outputs stay 3.3 V.
  - Power the MPU6050/GY-521 from 3.3 V in this build so I2C pull-ups cannot
    raise SDA/SCL above the ESP32 logic rail.
  - Servo power comes from the external regulated 5 V rail; only its signal
    wire connects to the ESP32.
  ...
  Target board:
    ESP32 NodeMCU / ESP32-WROOM-32 style development board.
  ...
  ESP32 pin map:
    ...
    GPIO26 -> L298N ENB (PWM)
    GPIO27 -> L298N IN1
    GPIO14 -> L298N IN2
    GPIO32 -> L298N IN3
    GPIO33 -> L298N IN4
    GPIO13 -> Buzzer
    GPIO18 -> Pan-servo signal
    GPIO16 <- Bluetooth TX -> UART2 RX (future)
    GPIO17 -> Bluetooth RX <- UART2 TX (future)
    GPIO21 -> MPU6050 SDA
    GPIO22 -> MPU6050 SCL
    GPIO34 <- IR front-left OUT
    GPIO35 <- IR front-right OUT
    GPIO36 <- IR rear-left OUT
    GPIO39 <- IR rear-right OUT
    GPIO23 -> HC-SR04 TRIG
    GPIO19 <- HC-SR04 ECHO through 5 V -> 3.3 V divider
...
// ---------------- Motor driver: L298N ----------------
const uint8_t PIN_ENA = 25;  // PWM - left motor pair
const uint8_t PIN_ENB = 26;  // PWM - right motor pair
const uint8_t PIN_IN1 = 27;
const uint8_t PIN_IN2 = 14;
const uint8_t PIN_IN3 = 32;
const uint8_t PIN_IN4 = 33;
...
// ---------------- IR sensors ----------------
const uint8_t PIN_IR_FRONT_L = 34;
const uint8_t PIN_IR_FRONT_R = 35;
const uint8_t PIN_IR_REAR_L  = 36;
const uint8_t PIN_IR_REAR_R  = 39;
...
const uint8_t PIN_ECHO = 19;  // MUST be reduced to 3.3 V
...
// External Bluetooth module on ESP32 UART2.
// GPIO17 = UART2 TX -> Bluetooth RX
// GPIO16 = UART2 RX <- Bluetooth TX
const uint8_t PIN_BT_RX = 16;
const uint8_t PIN_BT_TX = 17;
...
void setup() {
  Serial.begin(115200);
  Serial2.begin(BLUETOOTH_BAUD, SERIAL_8N1, PIN_BT_RX, PIN_BT_TX);
