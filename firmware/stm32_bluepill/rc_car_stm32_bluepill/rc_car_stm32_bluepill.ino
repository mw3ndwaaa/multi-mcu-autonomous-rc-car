/*
  Multi-MCU Autonomous RC Car - STM32F103C8T6 Blue Pill Version
  ----------------------------------------------------------------
  Functional port of the Arduino Mega baseline firmware using the official
  ...
  Features:
    ...
    - USART2 reserved/enabled for a future external Bluetooth module

  IMPORTANT 3.3 V LOGIC NOTE
  --------------------------
  Design all sensor signals as 3.3 V logic for the Blue Pill.
  - HC-SR04 ECHO MUST be level shifted/divided to 3.3 V.
  - If IR modules are powered from 5 V, level-shift/divide their OUT signals,
    or power compatible modules from 3.3 V.
  - Power the MPU6050/GY-521 from 3.3 V for a clean 3.3 V I2C bus.
  - Servo power comes from the external regulated 5 V rail; only its signal
    wire connects to the Blue Pill.

  L298N POWER NOTE
  ----------------
  The L298N onboard 5 V regulator is NOT used. Remove the 5V-EN jumper and
  power L298N logic from the regulated external 5 V rail. All grounds common.

  Timer allocation note:
    On the STM32duino Blue Pill variant the Servo library uses TIM2.
    Therefore ENA/ENB are deliberately placed on PA6/PA7 (TIM3 PWM) to keep
    motor PWM independent of the servo timer.

  Required software:
    - STM32 MCU based boards (STM32duino Arduino core)
    - MPU6050_light by rfetick
    - Servo is included with STM32duino

  Blue Pill pin map:
    PA6  -> L298N ENA (TIM3 PWM)
    PA7  -> L298N ENB (TIM3 PWM)
    PB12 -> L298N IN1
    PB13 -> L298N IN2
    PB14 -> L298N IN3
    PB15 -> L298N IN4
    ...
    PA2  -> USART2 TX -> Bluetooth RX (future)
    PA3  <- USART2 RX <- Bluetooth TX (future)
    PB7  -> MPU6050 SDA
    PB6  -> MPU6050 SCL
    PA4  <- IR front-left OUT
    PA5  <- IR front-right OUT
    PB10 <- IR rear-left OUT
    PB11 <- IR rear-right OUT
    PB0  -> HC-SR04 TRIG
    PB1  <- HC-SR04 ECHO through 5 V -> 3.3 V divider

  Debug serial:
    Generic Serial on the standard Blue Pill variant uses USART1 PA9/PA10
    unless USB CDC is enabled in the selected STM32duino board configuration.

  Safety design is intentionally kept equivalent to the Mega baseline.
*/
...
// ---------------- Motor driver: L298N ----------------
const uint32_t PIN_ENA = PA6;   // TIM3 PWM - left motor pair
const uint32_t PIN_ENB = PA7;   // TIM3 PWM - right motor pair
const uint32_t PIN_IN1 = PB12;
const uint32_t PIN_IN2 = PB13;
const uint32_t PIN_IN3 = PB14;
const uint32_t PIN_IN4 = PB15;
...
// ---------------- IR sensors ----------------
const uint32_t PIN_IR_FRONT_L = PA4;
const uint32_t PIN_IR_FRONT_R = PA5;
const uint32_t PIN_IR_REAR_L  = PB10;
const uint32_t PIN_IR_REAR_R  = PB11;

// Verified on this robot: LOW means obstacle.
const bool IR_ACTIVE_LOW = true;

// ---------------- Ultrasonic ----------------
const uint32_t PIN_TRIG = PB0;
const uint32_t PIN_ECHO = PB1;  // MUST be reduced to 3.3 V
...
// ---------------- Pan servo ----------------
const uint32_t PIN_SERVO_PAN = PA8;
...
// Measured on the current mechanical robot using the Mega baseline.
// Re-verify after swapping controllers:
...
// ---------------- Buzzer ----------------
const uint32_t PIN_BUZZER = PB8;

// ---------------- Bluetooth / command interface ----------------
// USART2 is kept separate from the default Serial/USART1 debug port.
// PA2 = TX -> Bluetooth RX
// PA3 = RX <- Bluetooth TX
const unsigned long BLUETOOTH_BAUD = 9600;

// ---------------- I2C ----------------
const uint32_t PIN_I2C_SDA = PB7;
const uint32_t PIN_I2C_SCL = PB6;
...
Servo panServo;
MPU6050 mpu(Wire);
HardwareSerial BluetoothSerial(PA3, PA2);  // RX, TX
...
void handleBluetoothCommands() {
  while (BluetoothSerial.available()) {
    ...
    processCommand(command, BluetoothSerial);
  }
}
